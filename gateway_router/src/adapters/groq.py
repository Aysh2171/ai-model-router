"""
Live Groq Provider Adapter Implementation.
Connects to Groq Ultra-Low Latency Inference API (https://api.groq.com/openai/v1) for real model execution.
"""
...
class GroqProviderAdapter(BaseProviderAdapter):
    """Concrete live provider adapter executing ultra-fast requests against Groq API."""
    ...
    def __init__(
        ...
        default_live_model: str = "groq/compound-mini",
        max_retries: int = 1,
        retry_delay: float = 2.0,
    ):
        self._provider = provider
        self._name = name or f"groq_{provider.lower()}_adapter"
    ...
    def is_configured(self) -> bool:
        key = self._api_key or os.environ.get("GROQ_API_KEY", "").strip()
    ...
    def _get_client(self) -> OpenAI:
        key = self._api_key or os.environ.get("GROQ_API_KEY", "").strip()
        if not key:
            raise PermanentExecutionError("GROQ_API_KEY is not configured in server environment.")
            ...
            try:
                ...
                if "429" in err_msg or isinstance(err, RateLimitError):
                    if attempt < max_retries:
                        ...
                        delay += 3.0
                        continue
                    raise TransientExecutionError(f"Groq API rate limit reached: {err}")
                elif "404" in err_msg:
                    raise PermanentExecutionError(f"Groq model not found: {err}")
                else:
                    raise PermanentExecutionError(f"Groq API error: {err}")
            except Exception as e:
                err_str = str(e).lower()
                if "timeout" in err_str:
                    raise TimeoutExecutionError(f"Groq API request timed out: {e}")
                raise TransientExecutionError(f"Groq API request failed: {e}")

        raise TransientExecutionError("Groq execution retries exhausted.")
    ...
    def execute_stream(self, request: GatewayRequest, model: RankedModel) -> Generator[StreamChunk, None, None]:
        ...
        try:
            ...
            raise TransientExecutionError(f"Groq streaming failed: {e}")

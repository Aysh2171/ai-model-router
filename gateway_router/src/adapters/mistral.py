"""
Live Mistral AI Provider Adapter Implementation.
Connects to Mistral AI API (https://api.mistral.ai/v1) for real model execution.
"""
...
class MistralProviderAdapter(BaseProviderAdapter):
    """Concrete live provider adapter executing requests against Mistral AI API."""
    ...
    def __init__(
        ...
        default_live_model: str = "mistral-small-latest",
        max_retries: int = 1,
        retry_delay: float = 2.0,
    ):
        self._provider = provider
        self._name = name or f"mistral_{provider.lower()}_adapter"
        self._base_url = base_url
        self._api_key = api_key or os.environ.get("MISTRAL_API_KEY", "").strip()
    ...
    def is_configured(self) -> bool:
        key = self._api_key or os.environ.get("MISTRAL_API_KEY", "").strip()
    ...
    def _get_client(self) -> OpenAI:
        key = self._api_key or os.environ.get("MISTRAL_API_KEY", "").strip()
        if not key:
            raise PermanentExecutionError("MISTRAL_API_KEY is not configured in server environment.")
            ...
            try:
                ...
                if "429" in err_msg or isinstance(err, RateLimitError):
                    if attempt < max_retries:
                        ...
                        delay += 3.0
                        continue
                    raise TransientExecutionError(f"Mistral API rate limit reached: {err}")
                elif "404" in err_msg:
                    raise PermanentExecutionError(f"Mistral model not found: {err}")
                else:
                    raise PermanentExecutionError(f"Mistral API error: {err}")
            except Exception as e:
                err_str = str(e).lower()
                if "timeout" in err_str:
                    raise TimeoutExecutionError(f"Mistral API request timed out: {e}")
                raise TransientExecutionError(f"Mistral API request failed: {e}")

        raise TransientExecutionError("Mistral execution retries exhausted.")
    ...
    def execute_stream(self, request: GatewayRequest, model: RankedModel) -> Generator[StreamChunk, None, None]:
        ...
        try:
            ...
            raise TransientExecutionError(f"Mistral streaming failed: {e}")
